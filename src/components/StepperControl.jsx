...
const StepperControl = ({handleClick, currentStep, steps}) => {
  return (
    <div className='container flex justify-around mt-4'>
      ...
      {/* next btn */}
      <button onClick={()=> handleClick("next")} className='bg-green-500 text-white py-2 px-4 rounded-xl font-semibold cursor-pointer  hover:bg-slate-700 hover:text-white transition duration-200 ease-in-out'>{currentStep === steps.length  ? " تایید و ثبت" : " مرحله بعد"}</button>
    </div>
  )
}
